...
namespace TackEngine.Android.Renderer {
    internal class AndroidShaderImpl : IShaderImplementation {
        ...
        public int CompileSubShader(string source, int type) {
            ...
            // Compile shader
            int subShaderId = GL.CreateShader((ShaderType)type == ShaderType.VertexShader ? ShaderType.VertexShader : ShaderType.FragmentShader);
        ...
        public void SetUniformValueColour(int id, string name, Colour4b colour) {
            OpenTK.Vector4 vec4tk = new OpenTK.Vector4(colour.R, colour.G, colour.B, colour.A);
            GL.Uniform4(GL.GetUniformLocation(id, name), ref vec4tk);
        }
        ...
        public Shader LoadShaderFromFile(string shaderName, Shader.ShaderContext context, string vertPath, string fragPath) {
            try {
                ...
                if (context == Shader.ShaderContext.Line) {
                    isVerified = true;
                }
